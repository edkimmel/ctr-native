...
int main(void) {
	...
	/* Closing an unfinalized session removes only its private temporary. */
	CHECK(Path(q));NativeReplayV4Record_Init(&rec);CHECK(NativeReplayV4Record_Open(&rec,q,&h.identity,&h.config));CHECK(Fill(&out,&f,0));CHECK(NativeReplayV4Record_AppendFrame(&rec,&f));NativeReplayV4Record_Close(&rec);NativeReplayV4Playback_Init(&play);CHECK(!NativeReplayV4Playback_Open(&play,q,&h.identity,&h.config,&out));
