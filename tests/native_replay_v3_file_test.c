...
static int Frame(const struct NativeReplayV3Header*h,uint32_t n,struct NativeReplayV3Frame*f){uint8_t st[NATIVE_CANONICAL_DRIVERS_NORMATIVE_BYTES]={0};memset(f,0,sizeof(*f));f->replayFrame=n;f->padCount=4;f->vsyncPacketCount=1;f->vsyncPackets[0]=1;f->vsyncTotal=1;NativeCanonicalStateV3_Init(&f->canonical);f->canonical.identity=h->identity;f->canonical.frameNumber=n;st[64]=1;if(!NativeCanonicalDriversV1_FromNormativeStream(&f->canonical.drivers,1,st))return 0;return NativeCanonicalStateV3_ComputeDigests(&f->canonical);}
