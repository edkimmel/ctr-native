...
static int PhysicsProjectionTest(void)
{
	...
	struct MainCanonicalDriversRosterRaceDynamicsActivePendingBotMetaPhysicsCandidate value,before;struct Driver *driver;struct sData *sd=&sdata_static;
	...
	driver=FD(&fixture,0);driver->currBlockTouching=&top.quads[1];driver->underDriver=&top.quads[2];driver->lastValid=NULL;driver->stepFlagSet=UINT32_C(0xc0ff);driver->quadBlockHeight=INT_MIN;driver->velocity.x=INT32_MIN;driver->velocity.y=2;driver->velocity.z=INT32_MAX;driver->normalVecUP.x=INT16_MIN;driver->normalVecUP.y=3;driver->normalVecUP.z=INT16_MAX;driver->rotCurr.w=INT16_MIN;driver->accel.z=INT16_MAX;
	if(!MainCanonicalDrivers_ExtractRosterRaceDynamicsActivePendingBotMetaPhysics(&fixture.tracker,sd,&context,&snapshot,&value)||value.physics[0].currQuadIndex!=1||value.physics[0].underDriverQuadIndex!=2||value.physics[0].lastValidQuadIndex!=UINT32_MAX||value.physics[0].terrainMeta1Index!=1||value.physics[0].terrainMeta2Index!=20||value.physics[0].velocity[0]!=INT32_MIN||value.physics[0].normalVecUP[0]!=INT16_MIN||value.physics[0].rotCurr[3]!=INT16_MIN||value.physics[0].accel[2]!=INT16_MAX)return 0;
