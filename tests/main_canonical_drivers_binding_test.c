...
union PhysicsPackStorage { uint64_t alignment; uint8_t bytes[8192]; };
struct PhysicsTopologyFixture { union PhysicsPackStorage storage; struct Level *level; struct mesh_info *mesh; struct QuadBlock *quads; };
static uintptr_t PhysicsAlign(uintptr_t value,size_t alignment){return (value+alignment-1u)&~(uintptr_t)(alignment-1u);}
static int PhysicsTopologyInit(struct SourceFixture *f,struct PhysicsTopologyFixture *top,struct MainCanonicalTopologyContext *context,struct MainCanonicalTopologySnapshot *snapshot)
{
	uintptr_t cursor,end;
	memset(top,0,sizeof(*top));cursor=(uintptr_t)top->storage.bytes;end=cursor+sizeof(top->storage.bytes);
	top->level=(struct Level *)(void *)PhysicsAlign(cursor,_Alignof(struct Level));cursor=(uintptr_t)(top->level+1);
	top->mesh=(struct mesh_info *)(void *)PhysicsAlign(cursor,_Alignof(struct mesh_info));cursor=(uintptr_t)(top->mesh+1);
	top->quads=(struct QuadBlock *)(void *)PhysicsAlign(cursor,_Alignof(struct QuadBlock));
	if((uintptr_t)(top->quads+3)>end)return 0;
	top->level->ptr_mesh_info=top->mesh;top->level->LevNavTable=f->navTable;top->mesh->ptrQuadBlockArray=top->quads;top->mesh->numQuadBlock=3;
	f->tracker.level1=top->level;sdata_static.mempack[0].start=top->storage.bytes;sdata_static.mempack[0].packSize=(int)sizeof(top->storage.bytes);
	MainCanonicalTopology_Init(context);return MainCanonicalTopology_Capture(context,snapshot,&f->tracker,&sdata_static);
}
static uint16_t PhysicsReadU16(const uint8_t *p){return (uint16_t)p[0]|((uint16_t)p[1]<<8);}
static uint32_t PhysicsReadU32(const uint8_t *p){return (uint32_t)p[0]|((uint32_t)p[1]<<8)|((uint32_t)p[2]<<16)|((uint32_t)p[3]<<24);}
static int DetailedFromPhysicsCandidate(const struct MainCanonicalDriversRosterRaceDynamicsActivePendingBotMetaPhysicsCandidate *candidate,struct NativeCanonicalDriversDetailedV1 *detailed)
{
	...
	{
		detailed->slots[slot].meta=candidate->meta[slot];detailed->slots[slot].race=candidate->race[slot];detailed->slots[slot].physics=candidate->physics[slot];
		detailed->slots[slot].dynamics=candidate->dynamics[slot];detailed->slots[slot].active=candidate->active[slot];detailed->slots[slot].pendingDamage=candidate->pendingDamage[slot];detailed->slots[slot].bot=candidate->bot[slot];
	}
	return NativeCanonicalDriversDetailedV1_Validate(detailed);
}
#define SET_PHYS_S32(v,b) do{(v).x=(b);(v).y=(b)+1;(v).z=(b)+2;}while(0)
#define SET_PHYS_S16(v,b) do{(v).x=(b);(v).y=(b)+1;(v).z=(b)+2;}while(0)
#define CHECK_PHYS_S32(a,b) do{if((a)[0]!=(b).x||(a)[1]!=(b).y||(a)[2]!=(b).z)return 0;}while(0)
#define CHECK_PHYS_S16(a,b) do{if((a)[0]!=(b).x||(a)[1]!=(b).y||(a)[2]!=(b).z)return 0;}while(0)
static int PhysicsProjectionTest(void)
{
	struct SourceFixture fixture;struct PhysicsTopologyFixture top;struct MainCanonicalTopologyContext context;struct MainCanonicalTopologySnapshot snapshot;
	struct MainCanonicalDriversRosterRaceDynamicsActivePendingBotMetaPhysicsCandidate value,changed,before;struct NativeCanonicalDriversDetailedV1 detailed,changedDetailed;struct NativeCanonicalDriversV1 summary,changedSummary;uint8_t bytes[NATIVE_CANONICAL_DRIVERS_NORMATIVE_BYTES];
	struct Driver *driver;struct sData *sd=&sdata_static;size_t offset;const int32_t *s32groups[4];const int16_t *s16groups[12];
	SourceFixtureInit(&fixture);if(!PhysicsTopologyInit(&fixture,&top,&context,&snapshot))return 0;
	for(uint8_t slot=0;slot<8;slot++)if(fixture.tracker.drivers[slot]){fixture.tracker.drivers[slot]->terrainMeta1=&data.MetaDataTerrain[1];fixture.tracker.drivers[slot]->terrainMeta2=&data.MetaDataTerrain[20];}
	driver=FD(&fixture,0);driver->currBlockTouching=&top.quads[1];driver->underDriver=&top.quads[2];driver->lastValid=NULL;driver->stepFlagSet=UINT32_C(0xc0ff);driver->quadBlockHeight=INT_MIN;
	SET_PHYS_S32(driver->velocity,INT32_MIN);SET_PHYS_S32(driver->originToCenter,-100);SET_PHYS_S32(driver->posCurr,100);SET_PHYS_S32(driver->posPrev,INT32_MAX-2);
	SET_PHYS_S16(driver->normalVecUP,INT16_MIN);SET_PHYS_S16(driver->spsHitPos,-300);SET_PHYS_S16(driver->spsNormalVec,-200);SET_PHYS_S16(driver->AxisAngle1_normalVec,-100);SET_PHYS_S16(driver->AxisAngle2_normalVec,100);SET_PHYS_S16(driver->AxisAngle3_normalVec,200);SET_PHYS_S16(driver->AxisAngle4_normalVec,300);SET_PHYS_S16(driver->posWallColl,400);SET_PHYS_S16(driver->forwardAccelVector,500);SET_PHYS_S16(driver->accel,INT16_MAX-2);driver->rotCurr.x=600;driver->rotCurr.y=601;driver->rotCurr.z=602;driver->rotCurr.w=603;driver->rotPrev.x=700;driver->rotPrev.y=701;driver->rotPrev.z=702;driver->rotPrev.w=703;
	if(!MainCanonicalDrivers_ExtractRosterRaceDynamicsActivePendingBotMetaPhysics(&fixture.tracker,sd,&context,&snapshot,&value)||value.physics[0].currQuadIndex!=1||value.physics[0].underDriverQuadIndex!=2||value.physics[0].lastValidQuadIndex!=UINT32_MAX||value.physics[0].terrainMeta1Index!=1||value.physics[0].terrainMeta2Index!=20||value.physics[0].quadBlockHeight!=INT_MIN)return 0;
	CHECK_PHYS_S32(value.physics[0].velocity,driver->velocity);CHECK_PHYS_S32(value.physics[0].originToCenter,driver->originToCenter);CHECK_PHYS_S32(value.physics[0].posCurr,driver->posCurr);CHECK_PHYS_S32(value.physics[0].posPrev,driver->posPrev);CHECK_PHYS_S16(value.physics[0].normalVecUP,driver->normalVecUP);CHECK_PHYS_S16(value.physics[0].spsHitPos,driver->spsHitPos);CHECK_PHYS_S16(value.physics[0].spsNormalVec,driver->spsNormalVec);CHECK_PHYS_S16(value.physics[0].axisAngle1,driver->AxisAngle1_normalVec);CHECK_PHYS_S16(value.physics[0].axisAngle2,driver->AxisAngle2_normalVec);CHECK_PHYS_S16(value.physics[0].axisAngle3,driver->AxisAngle3_normalVec);CHECK_PHYS_S16(value.physics[0].axisAngle4,driver->AxisAngle4_normalVec);CHECK_PHYS_S16(value.physics[0].posWallColl,driver->posWallColl);CHECK_PHYS_S16(value.physics[0].forwardAccelVector,driver->forwardAccelVector);CHECK_PHYS_S16(value.physics[0].accel,driver->accel);
	if(!DetailedFromPhysicsCandidate(&value,&detailed)||!EncodeDetailed(&detailed,bytes)||!NativeCanonicalDriversDetailedV1_BuildSummary(&detailed,&summary))return 0;
	offset=64u+40u+60u; if(PhysicsReadU32(bytes+offset)!=1||PhysicsReadU32(bytes+offset+4)!=2||PhysicsReadU32(bytes+offset+8)!=UINT32_MAX||bytes[offset+12]!=1||bytes[offset+13]!=20||PhysicsReadU32(bytes+offset+16)!=UINT32_C(0xc0ff)||PhysicsReadU32(bytes+offset+20)!=(uint32_t)INT_MIN)return 0;
	s32groups[0]=value.physics[0].velocity;s32groups[1]=value.physics[0].originToCenter;s32groups[2]=value.physics[0].posCurr;s32groups[3]=value.physics[0].posPrev;offset+=24;for(uint8_t group=0;group<4;group++)for(uint8_t n=0;n<3;n++,offset+=4)if(PhysicsReadU32(bytes+offset)!=(uint32_t)s32groups[group][n])return 0;
	s16groups[0]=value.physics[0].normalVecUP;s16groups[1]=value.physics[0].spsHitPos;s16groups[2]=value.physics[0].spsNormalVec;s16groups[3]=value.physics[0].axisAngle1;s16groups[4]=value.physics[0].axisAngle2;s16groups[5]=value.physics[0].axisAngle3;s16groups[6]=value.physics[0].axisAngle4;s16groups[7]=value.physics[0].rotCurr;s16groups[8]=value.physics[0].rotPrev;s16groups[9]=value.physics[0].posWallColl;s16groups[10]=value.physics[0].forwardAccelVector;s16groups[11]=value.physics[0].accel;
	for(uint8_t group=0;group<12;group++)for(uint8_t n=0;n<(group==7||group==8?4:3);n++,offset+=2)if(PhysicsReadU16(bytes+offset)!=(uint16_t)s16groups[group][n])return 0;
	driver->velocity.x=INT32_MAX;if(!MainCanonicalDrivers_ExtractRosterRaceDynamicsActivePendingBotMetaPhysics(&fixture.tracker,sd,&context,&snapshot,&changed)||!DetailedFromPhysicsCandidate(&changed,&changedDetailed)||!NativeCanonicalDriversDetailedV1_BuildSummary(&changedDetailed,&changedSummary)||summary.slots[0].metaRaceDigest!=changedSummary.slots[0].metaRaceDigest||summary.slots[0].physicsDynamicsDigest==changedSummary.slots[0].physicsDynamicsDigest||summary.slots[2].physicsDynamicsDigest!=changedSummary.slots[2].physicsDynamicsDigest||summary.fullStreamDigest==changedSummary.fullStreamDigest)return 0;driver->velocity.x=INT32_MIN;
	/* Each raw pointer is normalized independently, before any target memory is
	 * observed.  Each failure keeps the complete aggregate unchanged. */
	before=value;driver->currBlockTouching=(struct QuadBlock *)(uintptr_t)1;if(MainCanonicalDrivers_ExtractRosterRaceDynamicsActivePendingBotMetaPhysics(&fixture.tracker,sd,&context,&snapshot,&value)||memcmp(&value,&before,sizeof(value))!=0)return 0;driver->currBlockTouching=&top.quads[1];
	driver->underDriver=(struct QuadBlock *)(uintptr_t)1;if(MainCanonicalDrivers_ExtractRosterRaceDynamicsActivePendingBotMetaPhysics(&fixture.tracker,sd,&context,&snapshot,&value)||memcmp(&value,&before,sizeof(value))!=0)return 0;driver->underDriver=&top.quads[2];
	driver->lastValid=(struct QuadBlock *)(uintptr_t)1;if(MainCanonicalDrivers_ExtractRosterRaceDynamicsActivePendingBotMetaPhysics(&fixture.tracker,sd,&context,&snapshot,&value)||memcmp(&value,&before,sizeof(value))!=0)return 0;driver->lastValid=NULL;
	driver->terrainMeta1=(struct Terrain *)(uintptr_t)1;if(MainCanonicalDrivers_ExtractRosterRaceDynamicsActivePendingBotMetaPhysics(&fixture.tracker,sd,&context,&snapshot,&value)||memcmp(&value,&before,sizeof(value))!=0)return 0;driver->terrainMeta1=&data.MetaDataTerrain[1];
	driver->terrainMeta2=(struct Terrain *)(uintptr_t)1;if(MainCanonicalDrivers_ExtractRosterRaceDynamicsActivePendingBotMetaPhysics(&fixture.tracker,sd,&context,&snapshot,&value)||memcmp(&value,&before,sizeof(value))!=0)return 0;driver->terrainMeta2=&data.MetaDataTerrain[20];
	for(uint8_t bit=0;bit<32;bit++)if((UINT32_C(0x0000c0ff)&(UINT32_C(1)<<bit))==0){driver->stepFlagSet=UINT32_C(0xc0ff)|(UINT32_C(1)<<bit);if(MainCanonicalDrivers_ExtractRosterRaceDynamicsActivePendingBotMetaPhysics(&fixture.tracker,sd,&context,&snapshot,&value)||memcmp(&value,&before,sizeof(value))!=0)return 0;}driver->stepFlagSet=UINT32_C(0xc0ff);
	if(MainCanonicalDrivers_ExtractRosterRaceDynamicsActivePendingBotMetaPhysics(&fixture.tracker,sd,NULL,&snapshot,&value)||memcmp(&value,&before,sizeof(value))!=0||MainCanonicalDrivers_ExtractRosterRaceDynamicsActivePendingBotMetaPhysics(&fixture.tracker,sd,&context,NULL,&value)||memcmp(&value,&before,sizeof(value))!=0)return 0;
	{struct sData wrongSource=*sd;struct GameTracker wrongRoot=fixture.tracker;struct MainCanonicalTopologySnapshot stale=snapshot;wrongSource.gGT=&fixture.tracker;
		if(MainCanonicalDrivers_ExtractRosterRaceDynamicsActivePendingBotMetaPhysics(&fixture.tracker,&wrongSource,&context,&snapshot,&value)||memcmp(&value,&before,sizeof(value))!=0||MainCanonicalDrivers_ExtractRosterRaceDynamicsActivePendingBotMetaPhysics(&wrongRoot,sd,&context,&snapshot,&value)||memcmp(&value,&before,sizeof(value))!=0)return 0;
		FD(&fixture,5)->terrainMeta2=(struct Terrain *)(uintptr_t)1;if(MainCanonicalDrivers_ExtractRosterRaceDynamicsActivePendingBotMetaPhysics(&fixture.tracker,sd,&context,&snapshot,&value)||memcmp(&value,&before,sizeof(value))!=0)return 0;FD(&fixture,5)->terrainMeta2=&data.MetaDataTerrain[20];
		MainCanonicalTopology_Invalidate(&context);if(!MainCanonicalTopology_Capture(&context,&snapshot,&fixture.tracker,sd)||MainCanonicalDrivers_ExtractRosterRaceDynamicsActivePendingBotMetaPhysics(&fixture.tracker,sd,&context,&stale,&value)||memcmp(&value,&before,sizeof(value))!=0||!MainCanonicalDrivers_ExtractRosterRaceDynamicsActivePendingBotMetaPhysics(&fixture.tracker,sd,&context,&snapshot,&value))return 0;}
	/* A valid active capture is required even when every Physics QuadBlock
	 * reference is null.  The three nulls still have an unambiguous sentinel. */
	driver->currBlockTouching=NULL;driver->underDriver=NULL;driver->lastValid=NULL;
	if(!MainCanonicalDrivers_ExtractRosterRaceDynamicsActivePendingBotMetaPhysics(&fixture.tracker,sd,&context,&snapshot,&value)||
		value.physics[0].currQuadIndex!=UINT32_MAX||value.physics[0].underDriverQuadIndex!=UINT32_MAX||value.physics[0].lastValidQuadIndex!=UINT32_MAX)return 0;
	before=value;driver->stepFlagSet=UINT32_C(0x10000);if(MainCanonicalDrivers_ExtractRosterRaceDynamicsActivePendingBotMetaPhysics(&fixture.tracker,sd,&context,&snapshot,&value)||memcmp(&value,&before,sizeof(value))!=0)return 0;
	driver->stepFlagSet=0;driver->terrainMeta1=(struct Terrain *)(uintptr_t)1;if(MainCanonicalDrivers_ExtractRosterRaceDynamicsActivePendingBotMetaPhysics(&fixture.tracker,sd,&context,&snapshot,&value)||memcmp(&value,&before,sizeof(value))!=0)return 0;
	driver->terrainMeta1=&data.MetaDataTerrain[1];driver->currBlockTouching=(struct QuadBlock *)((uintptr_t)&top.quads[1]+1);if(MainCanonicalDrivers_ExtractRosterRaceDynamicsActivePendingBotMetaPhysics(&fixture.tracker,sd,&context,&snapshot,&value)||memcmp(&value,&before,sizeof(value))!=0)return 0;
	driver->currBlockTouching=&top.quads[1];MainCanonicalTopology_Invalidate(&context);if(MainCanonicalDrivers_ExtractRosterRaceDynamicsActivePendingBotMetaPhysics(&fixture.tracker,sd,&context,&snapshot,&value)||memcmp(&value,&before,sizeof(value))!=0)return 0;
	/* Rootless menu/reset observations remain exact zero and do not require a
	 * topology capture because no Driver or QuadBlock is inspected. */
	SourceFixtureInit(&fixture);memset(fixture.tracker.drivers,0,sizeof(fixture.tracker.drivers));memset(fixture.tracker.driversInRaceOrder,0,sizeof(fixture.tracker.driversInRaceOrder));
	fixture.tracker.numWinners=0;memset(sd->navBotList,0,sizeof(sd->navBotList));
	if(!MainCanonicalDrivers_ExtractRosterRaceDynamicsActivePendingBotMetaPhysics(&fixture.tracker,sd,NULL,NULL,&value)||
