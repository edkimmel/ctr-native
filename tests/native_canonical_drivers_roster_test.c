...
static int FullRoster(void)
{
	...
	Init(&i);for(uint8_t n=0;n<8;n++){Slot(&i,n,n<4?NATIVE_CANONICAL_DRIVER_KIND_HUMAN:NATIVE_CANONICAL_DRIVER_KIND_BOT);i.raceOrder[n]=(uint8_t)(7-n);if(n>=4)i.navOrder[0][n-4]=n;if(n<4){i.ranks[n]=(uint8_t)n;i.winnerDriverIDs[n]=n;}}
	i.playerCount=4;i.raceOrderCount=8;i.winnerCount=4;i.navCount[0]=4;
...
int main(void)
{
	...
	Init(&i);C(NativeCanonicalDriversRoster_Normalize(&i,&o)&&o.prelude.presenceMask==0&&o.prelude.raceOrderCount==0&&o.prelude.numLaps==0);
	...
	Slot(&i,4,NATIVE_CANONICAL_DRIVER_KIND_BOT);i.raceOrderCount=2;i.raceOrder[1]=4;i.navCount[0]=1;i.navOrder[0][0]=4;C(NativeCanonicalDriversRoster_Normalize(&i,&o)&&o.prelude.activeBotCount==1);
	i.slots[3].driverID=2;b=o;C(!NativeCanonicalDriversRoster_Normalize(&i,&o)&&memcmp(&o,&b,sizeof(o))==0);
	i.slots[3].driverID=3;i.numLaps=-1;C(!NativeCanonicalDriversRoster_Normalize(&i,&o)&&memcmp(&o,&b,sizeof(o))==0);
