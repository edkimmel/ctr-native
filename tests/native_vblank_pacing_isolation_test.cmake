# Structural isolation for the proof-only fixed VBlank pacing (R-6b;
...
#     defined once in platform/native_platform.c, each inside a CTR_INTERNAL
#     guard, and the definition only stores the normalized flag; main.c calls
#     it exactly once, as Platform_SetFixedVBlankPacing(1), inside the
#     roster proof's configure block and its CTR_INTERNAL guard, after the
#     proof was configured and its pads installed; no other file names it;
...
set(setter_owners "${platform_header_path}" "${platform_path}" "main.c")
...
ctr_require_guarded("${platform_header_path}" "${platform_h_code}" "Platform_SetFixedVBlankPacing" "#if defined(CTR_INTERNAL)")
...
ctr_require_guarded("${platform_path}" "${platform_code}" "Platform_SetFixedVBlankPacing" "#if defined(CTR_INTERNAL)")
