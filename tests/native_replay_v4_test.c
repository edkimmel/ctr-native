...
static int LoadGolden(uint8_t *bytes,size_t size)
{
	static const uint8_t sha256[32]={0xec,0x5b,0x84,0x23,0xbe,0x36,0x0e,0x8c,0x69,0xf4,0xb4,0xff,0x5d,0x1c,0x48,0x56,0xfb,0xbf,0x07,0xf3,0x1f,0x7b,0xe8,0x5e,0xa7,0x59,0xbf,0xee,0xa5,0x0f,0x31,0xa3};
	struct NativeSha256 sha; uint8_t actual[32]; FILE *file=fopen("tests/fixtures/crv4_crf4_golden.bin","rb");
	if(!file||fread(bytes,1,size,file)!=size||fgetc(file)!=EOF){if(file)fclose(file);return 0;}fclose(file);
	NativeSha256_Init(&sha);NativeSha256_Update(&sha,bytes,size);NativeSha256_Final(&sha,actual);return memcmp(actual,sha256,sizeof(actual))==0;
}
int main(void)
{
	...
	/* Independently checked-in complete CRV4+CRF4 bytes, pinned length/SHA-256/EOF. */
	CHECK(LoadGolden(copy,sizeof(copy)));
	NativeCodecWriter_Init(&w,bytes,sizeof(bytes),NULL); CHECK(NativeReplayV4Header_Encode(&w,&h)); CHECK(NativeReplayV4Frame_Encode(&w,&h,&f)); CHECK(w.offset==sizeof(bytes)&&!memcmp(bytes,copy,sizeof(bytes)));
	NativeCodecReader_Init(&r,copy,sizeof(copy)); CHECK(NativeReplayV4Header_Decode(&r,&h.identity,&d)); CHECK(NativeReplayV4Frame_Decode(&r,&d,&h.identity,&df)); CHECK(r.offset==sizeof(copy)); NativeCodecWriter_Init(&w,bytes,sizeof(bytes),NULL); CHECK(NativeReplayV4Header_Encode(&w,&d)&&NativeReplayV4Frame_Encode(&w,&d,&df)&&!memcmp(bytes,copy,sizeof(bytes)));
	/* Provisional construction is legal; playback acceptance is finalized-only and atomic. */
	d=h; d.flags=0; CHECK(NativeReplayV4Header_Validate(&d)); NativeCodecWriter_Init(&w,bytes,NATIVE_REPLAY_V4_HEADER_BYTES,NULL); CHECK(NativeReplayV4Header_Encode(&w,&d)); memset(&hbefore,0xa5,sizeof(hbefore)); CHECK(RejectH(bytes,NATIVE_REPLAY_V4_HEADER_BYTES,&h.identity,&hbefore)); memset(&fbefore,0xa5,sizeof(fbefore)); CHECK(RejectF(copy+NATIVE_REPLAY_V4_HEADER_BYTES,NATIVE_REPLAY_V4_FRAME_BYTES,&d,&fbefore)); memcpy(bytes,copy,sizeof(bytes));
	...
	d=h; d.config.trackID=2; CHECK(NativeReplayV4Header_Validate(&d)); memset(&fbefore,0xa5,sizeof(fbefore)); CHECK(RejectF(bytes+428,1752,&d,&fbefore));
