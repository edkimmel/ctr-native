...
static void ValidHuman(struct NativeCanonicalDriversDetailedV1 *value)
{
	...
	value->prelude.presenceMask=1; value->prelude.raceOrder[0]=0; value->prelude.playerCount=1; value->prelude.humanPlayerPositions[0]=0; value->prelude.numLaps=3;
...
static int TestGoldenAndSummary(void)
{
	...
	CHECK(bytes[0]==8&&bytes[1]==0&&bytes[4]==1&&bytes[8]==0&&bytes[9]==0xff&&bytes[16]==1&&bytes[18]==3&&bytes[24]==0&&bytes[25]==0xff);
	...
	CHECK(summary.fullStreamDigest==UINT64_C(0xb80456806184b520));
	return 0;
}
...
int main(void)
{
	if(TestGoldenAndSummary()!=0||TestSemanticMutations()!=0||TestRejectionAndTransaction()!=0||TestBotAndReferences()!=0)return 1;
