...
# (A whole tab-indented line; its line end may be LF or CRLF, which the
# pattern's self-check proves whatever file(READ) does with a CR.)
set(tick_send_pattern "\n\tNativeArcadeNetplay_SendLaunch\\(netplay, 0\\);\r?\n")
string(REGEX MATCH "${tick_send_pattern}" tick_send_lf "x\n\tNativeArcadeNetplay_SendLaunch(netplay, 0);\ny")
string(REGEX MATCH "${tick_send_pattern}" tick_send_crlf "x\r\n\tNativeArcadeNetplay_SendLaunch(netplay, 0);\r\ny")
string(REGEX MATCH "${tick_send_pattern}" tick_send_trailing "x\n\tNativeArcadeNetplay_SendLaunch(netplay, 0); y\n")
if("${tick_send_lf}" STREQUAL "" OR "${tick_send_crlf}" STREQUAL "" OR NOT "${tick_send_trailing}" STREQUAL "")
    message(FATAL_ERROR "arcade netplay isolation: the Tick send pattern must match its line with an LF or a CRLF end, and nothing else on the line")
endif()
string(REGEX MATCH "${tick_send_pattern}" tick_send_line "${netplay_source}")
if("${tick_send_line}" STREQUAL "")
    message(FATAL_ERROR "arcade netplay isolation: Tick must send its launch record with NativeArcadeNetplay_SendLaunch(netplay, 0); on a line of its own (the capped rule)")
endif()
