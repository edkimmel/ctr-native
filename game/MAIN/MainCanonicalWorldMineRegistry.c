...
static uint32_t MineCapacity(const struct GameTracker *gGT)
{
...
static int SlotIndex(const struct OverlayDATA_231 *source, uint32_t capacity,
...
static int ReadList(const struct OverlayDATA_231 *source, const struct LinkedList *list,
	...
	struct NativeCanonicalWorldMineRegistryV1Facts *facts, int taken)
{
	...
	for (uint32_t ordinal = 0; ordinal < count; ++ordinal) {
		if (item == NULL || !SlotIndex(source, capacity, item, &index) || seen[index]) return 0;
	...
	struct NativeCanonicalWorldMineRegistryV1 *out)
{
	...
	if (!ReadList(source, &source->minePoolFree, capacity, seen, &freeCount, &facts, 0) ||
		!ReadList(source, &source->minePoolTaken, capacity, seen, &takenCount, &facts, 1) ||
