...
/* One race tick's lifecycle (see the header). */
	...
	struct MainArcadeRaceDigestTick *out)
{
	...
	if ((sources == NULL) || (out == NULL) || (sources->gGT == NULL) || (sources->sourceData == NULL) ||
	    (sources->mineSource == NULL) || (sources->config == NULL) || (sources->bank == NULL) || (sources->input == NULL))
	{
		return MainArcadeRaceDigest_Fail(MAIN_ARCADE_RACE_DIGEST_FAILURE_ARGUMENT);
	}
	if (raceTick == 0u)
	...
	struct MainArcadeRaceDigestTick *out)
{
	...
	projected = MainArcadeRaceDigest_ProjectTick(raceTick, sources, out);
