...
static int CopyPayload(const struct Span raw,uint32_t bigLba,const struct BigEntry *e,uint8_t *dst){uint32_t left=e->size;uint32_t sec=0;while(left){size_t base;size_t take=left>2048?2048:left;if(!Add((size_t)bigLba+e->offset+sec,0,&base)||!Mul(base,NATIVE_TOPOLOGY_ASSET_RAW_SECTOR_BYTES,&base)||base>raw.n||NATIVE_TOPOLOGY_ASSET_RAW_USER_OFFSET>raw.n-base||take>raw.n-base-NATIVE_TOPOLOGY_ASSET_RAW_USER_OFFSET||!RawHeader(raw.p+base))return 0;memcpy(dst,raw.p+base+NATIVE_TOPOLOGY_ASSET_RAW_USER_OFFSET,take);dst+=take;left-=take;sec++;}return 1;}
...
static int FindBig(const struct Span raw,uint32_t *lba,uint32_t *size){size_t pvd=(size_t)16*NATIVE_TOPOLOGY_ASSET_RAW_SECTOR_BYTES+24;const uint8_t *p;uint32_t rootLba,rootSize; if(pvd+2048>raw.n||!RawHeader(raw.p+16*2352))return 0;p=raw.p+pvd;if(p[0]!=1||memcmp(p+1,"CD001",5)||p[6]!=1)return 0;rootLba=U32(p+158);rootSize=U32(p+166);for(uint32_t sec=0;sec<(rootSize+2047)/2048;sec++){size_t base=((size_t)rootLba+sec)*2352+24;size_t pos=0;if(base<24||base+2048>raw.n||!RawHeader(raw.p+base-24))return 0;while(pos<2048){const uint8_t *d=raw.p+base+pos;uint8_t n=d[0];if(!n)break;if(n<34||pos+n>2048)return 0;if(d[32]==13&&!memcmp(d+33,"BIGFILE.BIG;1",13)){*lba=U32(d+2);*size=U32(d+10);return 1;}pos+=n;}}return 0;}
