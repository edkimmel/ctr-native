...
int NativeReplayV4Record_Open(struct NativeReplayV4RecordSession *s,const char *p,const struct NativeIdentityV1 *i,const struct NativeMatchConfigV1 *c) { struct NativeReplayV4Header h;uint8_t b[NATIVE_REPLAY_V4_HEADER_BYTES];char *target,*temp;FILE *f=NULL;if(!s||s->stream||!p||!i||!c)return 0;target=CopyPath(p);if(!target)return 0; do { temp=TempPath(p); if(!temp){free(target);return 0;} f=CreateNew(temp); if(!f)free(temp); } while(!f); NativeReplayV4Header_Init(&h);h.identity=*i;h.config=*c;if(!NativeReplayV4Header_Validate(&h)||!EH(&h,b)||fwrite(b,1,sizeof(b),f)!=sizeof(b)||!Sync(f)){(void)fclose(f);(void)remove(temp);free(target);free(temp);return 0;}s->stream=f;s->header=h;s->path=target;s->temporaryPath=temp;return 1; }
